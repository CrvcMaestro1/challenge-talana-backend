...
class MockJourneyRepository(repositories.JourneyRepository):
    ...
    def finish_journey(self, journey: models.Journey) -> models.Journey:
...
class TestStopJourney:
    ...
    def test_stop(self):
        ...
        data = {"name": "Kitt", "passengers": 2}
        start_usecase = usecases.StartJourney(repo, notifier).set_params(data)
        journey = start_usecase.execute()

        stopped_journey = usecases.StopJourney(repo).set_params(journey).execute()

        assert stopped_journey.end is not None
